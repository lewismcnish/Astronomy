...
def make_template(m1,m2,fs,T,inv_psd,d=None,phic=None,tc=None):
    ...
    # add error checking on the masses
    if m1<m2:
        raise ValueError(
            'the primary mass m1 must always be greater than the secondary mass m2. Exiting.'
        )
    if m1/m2>8.0:
        raise ValueError(
            'You are asking for a mass ratio m1/m2>8. Waveforms in this regime are not trustworthy. '
            'Please restrict the ratio to be bewteen 1 and 8. Exiting.'
        )
